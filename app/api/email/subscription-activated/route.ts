...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await sendSubscriptionActivatedEmail(decodedToken.uid);
    return NextResponse.json(result);
  } catch (error) {
    return handleApiError(error, "Send subscription email failed", "Unable to send subscription email.");
  }
}
